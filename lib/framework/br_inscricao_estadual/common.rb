module BrInscricaoEstadual
  class Common
    ...
    def calculate(peso)
      @calc = []
      insc_est.split('').collect(&:to_i).take(peso.size).each_with_index { |item, index| @calc << (item * peso[index]) }
      @calc.inject(:+)
    end
    ...
    def state_code
      return insc_est[0..1] == self.class::STATE_CODE if self.class::STATE_CODE.is_a?(String)
      return self.class::STATE_CODE.include?(insc_est[0..1]) if self.class::STATE_CODE.is_a?(Array)
    end
    ...
    def digit_verify
      ...
      return false unless insc_est[(self.class::DIGITO_LEVEL1)].to_i == digito1 # [0,1].include?(digito1) &&
      ...
      if self.class.const_defined?('PESO2')
        ...
        return false unless insc_est[(self.class::DIGITO_LEVEL2)].to_i == digito2 # [0,2].include?(digito2) &&
      end
    ...
    def size_verify
      insc_est.size == self.class::STATE_INSC_SIZE
    end
  end
end
